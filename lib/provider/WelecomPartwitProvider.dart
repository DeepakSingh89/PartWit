...
class WelcomePartWitProvider extends ChangeNotifier{
  var wlcm = WelcomePartwitModel();
  bool loading = false;

  void  wlcmData(BuildContext context) async {
    try{

      loading = false;
      notifyListeners();
      var response = await  ApiServices.welcomPartWit(context);
      if (response != null) {
        loading = true;
        notifyListeners();
